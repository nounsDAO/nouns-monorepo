import { N00unsDAOV2ABI, N00unsDaoLogicV2Factory } from '@n00uns/sdk';
import {
  ChainId,
...
const n00unsDaoContract = new N00unsDaoLogicV2Factory().attach(config.addresses.n00unsDAOProxy);
...
const proposalCreatedFilter = {
  ...n00unsDaoContract.filters?.ProposalCreated(
    null,
    null,
    null,
    null,
...
export const useHasVotedOnProposal = (proposalId: string | undefined): boolean => {
  ...
  const [receipt] =
    useContractCall<[any]>({
      abi,
      address: n00unsDaoContract.address,
      method: 'getReceipt',
...
export const useCastRefundableVote = () => {
  ...
  return {
    castRefundableVote: async (...args: any[]): Promise<void> => {
      const contract = connectContractToSigner(n00unsDaoContract as any, undefined, library);
...
export const usePropose = () => {
  const { send: propose, state: proposeState } = useContractFunction(n00unsDaoContract as any, 'propose');
