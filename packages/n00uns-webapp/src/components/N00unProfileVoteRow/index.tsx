...
import { ProposalState } from '../../wrappers/n00unsDao';

import classes from './N00unProfileVoteRow.module.css';
...
import { N00unVoteHistory } from '../ProfileActivityFeed';
...
interface N00unProfileVoteRowProps {
  proposal: Proposal;
  vote?: N00unVoteHistory;
}

const selectIconForN00unVoteActivityRow = (proposal: Proposal, vote?: N00unVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: N00unVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
...
const N00unProfileVoteRow: React.FC<N00unProfileVoteRowProps> = props => {
  ...
  return (
    <tr onClick={proposalOnClickHandler} className={classes.voteInfoRow}>
      <td className={classes.voteIcon}>{selectIconForN00unVoteActivityRow(proposal, vote)}</td>
          ...
          {vote && vote.voter && (
            ...
            >
              ...
              <div
                ...
                data-for="n00un-profile-delegate"
                style={{
                  borderRadius: '8px',
...
export default N00unProfileVoteRow;
