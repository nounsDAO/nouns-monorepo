...
import classes from './VrbProfileVoteRow.module.css';
...
import { VrbVoteHistory } from '../ProfileActivityFeed';
...
interface VrbProfileVoteRowProps {
  proposal: Proposal;
  vote?: VrbVoteHistory;
}

const selectIconForVrbVoteActivityRow = (proposal: Proposal, vote?: VrbVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: VrbVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
...
const VrbProfileVoteRow: React.FC<VrbProfileVoteRowProps> = props => {
  ...
  return (
    <tr onClick={proposalOnClickHandler} className={classes.voteInfoRow}>
      <td className={classes.voteIcon}>{selectIconForVrbVoteActivityRow(proposal, vote)}</td>
...
export default VrbProfileVoteRow;
