...
import classes from './VrbInfoRowHolder.module.css';
...
interface VrbInfoRowHolderProps {
  vrbId: number;
}

const VrbInfoRowHolder: React.FC<VrbInfoRowHolderProps> = props => {
  const { vrbId } = props;
  const isCool = useAppSelector(state => state.application.isCoolBackground);
  const { loading, error, data } = useQuery(auctionQuery(vrbId));

  const winner = data && data.auction.bidder?.id;

  if (loading || !winner) {
    return (
      <div className={classes.vrbHolderInfoContainer}>
        <span className={classes.vrbHolderLoading}>
          <Trans>Loading...</Trans>
        </span>
      </div>
    );
    ...
    >
      <div className={classes.vrbHolderInfoContainer}>
        ...
        <span>
          <a
            className={
              isCool ? classes.vrbHolderEtherscanLinkCool : classes.vrbHolderEtherscanLinkWarm
            }
            href={etherscanURL}
            target={'_blank'}
            rel="noreferrer"
          >
            {winner.toLowerCase() === config.addresses.vrbsAuctionHouseProxy.toLowerCase() ? (
              <Trans>Vrbs Auction House</Trans>
            ) : (
...
export default VrbInfoRowHolder;
