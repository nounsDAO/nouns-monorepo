...
const proposalCreatedFilter = {
  ...vrbsDaoContract.filters?.ProposalCreated(
    null,
    null,
    null,
    null,
...
export const useHasVotedOnProposal = (proposalId: string | undefined): boolean => {
  ...
  const [receipt] =
    useContractCall<[any]>({
      abi,
      address: vrbsDaoContract.address,
      method: 'getReceipt',
...
export const useCastRefundableVote = () => {
  ...
  return {
    castRefundableVote: async (...args: any[]): Promise<void> => {
      const contract = connectContractToSigner(vrbsDaoContract as any, undefined, library);
...
export const usePropose = () => {
  const { send: propose, state: proposeState } = useContractFunction(vrbsDaoContract as any, 'propose');
