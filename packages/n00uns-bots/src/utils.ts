...
import { n00unsTokenContract } from './clients';
...
export function formatAuctionStartedTweetText(auctionId: number) {
 ...
 Learn more at https://n00uns.wtf`;
}
...
export function getAuctionEndingSoonTweetText() {
  return `This auction is ending soon! Bid now at https://n00uns.wtf`;
}
...
export async function getN00unPngBuffer(tokenId: string): Promise<Buffer | undefined> {
  const dataURI = await tryF(() => n00unsTokenContract.dataURI(tokenId));
  if (isError(dataURI)) {
