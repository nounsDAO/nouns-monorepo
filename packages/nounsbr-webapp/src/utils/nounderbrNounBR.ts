...
export const isNounderBRNounBR = (nounbrId: BigNumber) => {
  return nounbrId.mod(10).eq(0) || nounbrId.eq(0);
};

const emptyNounderBRAuction = (onDisplayAuctionId: number): Auction => {
  return {
 ...
 */
export const generateEmptyNounderBRAuction = (
  nounbrId: BigNumber,
  pastAuctions: AuctionState[],
): Auction => {
  const nounderbrAuction = emptyNounderBRAuction(nounbrId.toNumber());
