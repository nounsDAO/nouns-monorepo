...
import { generateEmptyNounderBRAuction, isNounderBRNounBR } from '../utils/nounderbrNounBR';
...
const useOnDisplayAuction = (): Auction | undefined => {
  ...
  // nounderbr auction
  if (isNounderBRNounBR(BigNumber.from(onDisplayAuctionNounBRId))) {
    const emptyNounderBRAuction = generateEmptyNounderBRAuction(
      BigNumber.from(onDisplayAuctionNounBRId),
      pastAuctions,
    );

    return deserializeAuction(emptyNounderBRAuction);
  }
