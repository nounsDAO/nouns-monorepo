...
export class DiscordAuctionLifecycleHandler implements IAuctionLifecycleHandler {
  ...
  async handleNewAuction(auctionId: number) {
    ...
    if (png) {
      ...
      const message = new Discord.MessageEmbed()
        ...
        .setDescription(`An auction has started for NounBR #${auctionId}`)
        .setURL('https://nounsbr.wtf')
        .addField('NounBR ID', auctionId, true)
   ...
   * @param auctionId NounBR auction number
  ...
  async handleNewProposal(proposal: Proposal) {
    const message = new Discord.MessageEmbed()
      ...
      .setURL(`https://nounsbr.wtf/vote/${proposal.id}`)
