...
const nounsExecutors: Record<number, string> = {
  [ChainId.Mainnet]: '0x0BC3807Ec262cB779b38D65b38158acC3bfedE10',
  [ChainId.Rinkeby]: '0x68136BF5dB4407DD599960585dA9b7746eCF567F',
};
const noundersSafe: Record<number, string> = {
  [ChainId.Mainnet]: '0x3808429E985F0B06878c74089831865E5d82A584',
  [ChainId.Rinkeby]: '0x68136BF5dB4407DD599960585dA9b7746eCF567F',
};
...
task('deploy', 'Deploys NFTDescriptor, NounsDescriptor, NounsSeeder, and NounsToken')
  ...
  .addOptionalParam(
    ...
    BigNumber.from('330000000000000000') /* 0.33 ETH */,
    types.int,
  )
  .addOptionalParam(
  ...
  .setAction(async (args, { ethers }) => {
    ...
    if (!args.noundersdao) {
      const noundersDao = noundersSafe[network.chainId];
      if (!noundersDao) {
        throw new Error(
          `Can not auto-detect nounders DAO contract on chain ${network.name}. Provide it with the --noundersdao arg.`,
        );
      }
      console.log(
        `Nounders DAO address not provided. Setting to default (${noundersDao})...`,
      );
      args.noundersdao = noundersDao;
    }
    ...
    if (!args.nounsExecutor) {
      const nounsExecutor = nounsExecutors[network.chainId];
      if (!nounsExecutor) {
        throw new Error(
          `Can not auto-detect nouns executor contract on chain ${network.name}. Provide it with the --nounsexecutor arg.`,
        );
      }
      args.nounsexecutor = nounsExecutor;
    }
        ...
        args: [
          ...
          () =>
            new Interface(NounsAuctionHouseABI).encodeFunctionData('initialize', [
              ...
              args.auctionReservePrice, // todo reserve price
