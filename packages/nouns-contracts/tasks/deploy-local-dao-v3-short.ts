...
task('deploy-local-dao-v3-short', 'Deploy contracts to hardhat')
  ...
  .addOptionalParam('auctionDuration', 'The auction duration (seconds)', 30, types.int) // Default: 30 second
  .addOptionalParam('timelockDelay', 'The timelock delay (seconds)', 60 * 2 /* 1 min */, types.int)
  .addOptionalParam(
    'votingPeriod',
  ...
  .addOptionalParam('votingDelay', 'The voting delay (blocks)', 20, types.int) // Default: 1 block
  .addOptionalParam(
  ...
  .setAction(async (args, { ethers }) => {
    ...
    const contracts: Record<LocalContractName, Contract> = {
      ...
      NounsDAOLogicV3: {
        libraries: () => ({
          NounsDAOV3Admin: contracts.NounsDAOV3AdminShortTimes.instance?.address as string,
      ...
      NounsDAOExecutorProxy: {
        args: [
          () => contracts.NounsDAOExecutorV2ShortTimes.instance?.address,
          () =>
