...
task(
  ...
  'Populates the descriptor with color palettes and Punk parts; accepts an input JSON with missing properies.',
)
  .addParam('nDescriptor', 'The `NDescriptor` contract address')
  .addParam('daoAddress', 'The `NDAOProxy` contract address')
  ...
  .setAction(
    async ({ nDescriptor, daoAddress, imageDataPath, proposalTextPath }, { ethers }) => {
      ...
      const dao = (await ethers.getContractFactory('NDAOLogicV1')).attach(daoAddress);
