...
describe('Token', () => {
  ...
  beforeEach(async () => {
    ...
    token = await deploy('Token', [root]);
  });

  describe('metadata', () => {
    it('has given name', async () => {
      expect(await call(token, 'name')).toEqual(name);
    });

    it('has given symbol', async () => {
      expect(await call(token, 'symbol')).toEqual(symbol);
    });
  });

  describe('balanceOf', () => {
    it('grants to initial account', async () => {
      expect(await call(token, 'balanceOf', [root])).toEqual("10000000000000000000000000");
    });
  });
  ...
  describe('numCheckpoints', () => {
    it('returns the number of checkpoints for a delegate', async () => {
      let guy = accounts[0];
      await send(token, 'transfer', [guy, '100']); //give an account a few tokens for readability
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('0');

      const t1 = await send(token, 'delegate', [a1], { from: guy });
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('1');

      const t2 = await send(token, 'transfer', [a2, 10], { from: guy });
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('2');

      const t3 = await send(token, 'transfer', [a2, 10], { from: guy });
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('3');

      const t4 = await send(token, 'transfer', [guy, 20], { from: root });
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('4');

      await expect(call(token, 'checkpoints', [a1, 0])).resolves.toEqual(expect.objectContaining({ fromBlock: t1.blockNumber.toString(), votes: '100' }));
      await expect(call(token, 'checkpoints', [a1, 1])).resolves.toEqual(expect.objectContaining({ fromBlock: t2.blockNumber.toString(), votes: '90' }));
      await expect(call(token, 'checkpoints', [a1, 2])).resolves.toEqual(expect.objectContaining({ fromBlock: t3.blockNumber.toString(), votes: '80' }));
      await expect(call(token, 'checkpoints', [a1, 3])).resolves.toEqual(expect.objectContaining({ fromBlock: t4.blockNumber.toString(), votes: '100' }));
    });
    ...
    it('does not add more than one checkpoint in a block', async () => {
      ...
      let t1 = send(token, 'delegate', [a1], { from: guy });
      let t2 = send(token, 'transfer', [a2, 10], { from: guy });
      let t3 = send(token, 'transfer', [a2, 10], { from: guy });
      ...
      await expect(call(token, 'checkpoints', [a1, 0])).resolves.toEqual(expect.objectContaining({ fromBlock: t1.blockNumber.toString(), votes: '80' }));
      await expect(call(token, 'checkpoints', [a1, 1])).resolves.toEqual(expect.objectContaining({ fromBlock: '0', votes: '0' }));
      await expect(call(token, 'checkpoints', [a1, 2])).resolves.toEqual(expect.objectContaining({ fromBlock: '0', votes: '0' }));

      const t4 = await send(token, 'transfer', [guy, 20], { from: root });
      await expect(call(token, 'numCheckpoints', [a1])).resolves.toEqual('2');
      await expect(call(token, 'checkpoints', [a1, 1])).resolves.toEqual(expect.objectContaining({ fromBlock: t4.blockNumber.toString(), votes: '100' }));
    });
  });
