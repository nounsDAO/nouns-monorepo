...
describe('NounsDescriptorV2', () => {
  ...
  it('should generate valid token uri metadata when data uris are enabled [ @skip-on-coverage ]', async () => {
    ...
    expect(name).to.equal('Public Noun 0');
    expect(description).to.equal('Public Noun 0 is a member of the Public Nouns DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      ...
      expect(name).to.equal(`Public Noun ${i}`);
      expect(description).to.equal(`Public Noun ${i} is a member of the Public Nouns DAO`);
      expect(image).to.not.be.undefined;
