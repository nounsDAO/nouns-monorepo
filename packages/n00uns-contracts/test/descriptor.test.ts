...
import { N00unsDescriptor } from '../typechain';
import ImageData from '../files/image-data-v1.json';
import { LongestPart } from './types';
import { deployN00unsDescriptor, populateDescriptor } from './utils';
...
describe('N00unsDescriptor', () => {
  let n00unsDescriptor: N00unsDescriptor;
  ...
  before(async () => {
    n00unsDescriptor = await deployN00unsDescriptor();
    ...
    await populateDescriptor(n00unsDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are disabled', async () => {
    const BASE_URI = 'https://api.n00uns.wtf/metadata/';

    await n00unsDescriptor.setBaseURI(BASE_URI);
    await n00unsDescriptor.toggleDataURIEnabled();

    const tokenUri = await n00unsDescriptor.tokenURI(0, {
      background: 0,
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await n00unsDescriptor.tokenURI(i, {
