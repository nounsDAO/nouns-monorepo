...
export function handleAuctionCreated(event: AuctionCreated): void {
  ...
  if (noun == null) {
    log.error('[handleAuctionCreated] Nouns #{} not found. Hash: {}', [
      nounId,
...
export function handleAuctionBid(event: AuctionBid): void {
  ...
  if (auction == null) {
    log.error('[handleAuctionBid] Auction not found for Nouns #{}. Hash: {}', [
      nounId,
      event.transaction.hash.toHex(),
    ]);
    return;
...
export function handleAuctionExtended(event: AuctionExtended): void {
  ...
  const auction = Auction.load(nounId);
  if (auction == null) {
    log.error('[handleAuctionExtended] Auction not found for Nouns #{}. Hash: {}', [
      nounId,
      event.transaction.hash.toHex(),
    ]);
    return;
...
export function handleAuctionSettled(event: AuctionSettled): void {
  ...
  const auction = Auction.load(nounId);
  if (auction == null) {
    log.error('[handleAuctionSettled] Auction not found for Nouns #{}. Hash: {}', [
      nounId,
      event.transaction.hash.toHex(),
    ]);
    return;
