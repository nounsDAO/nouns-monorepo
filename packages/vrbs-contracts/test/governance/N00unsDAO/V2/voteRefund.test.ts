...
import {
  N00unsDAOLogicV2,
  N00unsDAOLogicV2__factory,
  N00unsDescriptorV2__factory,
  N00unsToken,
  Voter__factory,
} from '../../../../typechain';
...
describe('Vote Refund', () => {
  before(async () => {
    ...
    token = await deployN00unsToken(deployer);
    const descriptor = N00unsDescriptorV2__factory.connect(await token.descriptor(), deployer);
  ...
  function expectRefundEvent(r: ContractReceipt, u: SignerWithAddress, expectedCost: BigNumber) {
    ...
    const daoInterface = N00unsDAOLogicV2__factory.createInterface();
