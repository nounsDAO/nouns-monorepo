...
describe('Vote Refund', () => {
  before(async () => {
    ...
    token = await deployVrbsToken(deployer);
    const descriptor = VrbsDescriptorV2__factory.connect(await token.descriptor(), deployer);
  ...
  function expectRefundEvent(r: ContractReceipt, u: SignerWithAddress, expectedCost: BigNumber) {
    ...
    const daoInterface = DAOLogicV2__factory.createInterface();
