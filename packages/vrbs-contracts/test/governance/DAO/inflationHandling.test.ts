...
import { VrbsToken, DAOLogicV1 } from '../../../typechain';
...
describe('VrbsDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on Vrb total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('VrbsDAO::propose: proposer votes below proposal threshold');
  });
