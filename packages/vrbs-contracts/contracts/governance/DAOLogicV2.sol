...
/// @title The Vrbs DAO logic version 2
...
// LICENSE
// DAOLogicV2.sol is a modified version of Compound Lab's GovernorBravoDelegate.sol:
...
//
// MODIFICATIONS
// See DAOLogicV1 for initial GovernorBravoDelegate modifications.
...
// max quorum votes using the current Vrb supply.
...
contract DAOLogicV2 is DAOStorageV2, DAOEventsV2 {
     ...
     * @param timelock_ The address of the DAOExecutor
     * @param vrbs_ The address of the NOUN tokens
    ...
    function initialize(
        ...
        require(timelock_ != address(0), 'VrbsDAO::initialize: invalid timelock address');
        require(vrbs_ != address(0), 'VrbsDAO::initialize: invalid vrbs address');
        require(
            votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD,
            'VrbsDAO::initialize: invalid voting period'
        );
        require(
            votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY,
            'VrbsDAO::initialize: invalid voting delay'
        );
        require(
            proposalThresholdBPS_ >= MIN_PROPOSAL_THRESHOLD_BPS && proposalThresholdBPS_ <= MAX_PROPOSAL_THRESHOLD_BPS,
            'VrbsDAO::initialize: invalid proposal threshold bps'
        );
    ...
    function propose(
        ...
        temp.totalSupply = vrbs.totalSupply();

        temp.proposalThreshold = bps2Uint(proposalThresholdBPS, temp.totalSupply);

        require(
            vrbs.getPriorVotes(msg.sender, block.number - 1) > temp.proposalThreshold,
            'VrbsDAO::propose: proposer votes below proposal threshold'
        );
        require(
            targets.length == values.length &&
                targets.length == signatures.length &&
                targets.length == calldatas.length,
            'VrbsDAO::propose: proposal function information arity mismatch'
        );
        require(targets.length != 0, 'VrbsDAO::propose: must provide actions');
        require(targets.length <= proposalMaxOperations, 'VrbsDAO::propose: too many actions');
        ...
        if (temp.latestProposalId != 0) {
            ...
            require(
                ...
                'VrbsDAO::propose: one live proposal per proposer, found an already active proposal'
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                'VrbsDAO::propose: one live proposal per proposer, found an already pending proposal'
            );
        }
    ...
    function queue(uint256 proposalId) external {
        require(
            ...
            'VrbsDAO::queue: proposal can only be queued if it is succeeded'
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            ...
            'VrbsDAO::queueOrRevertInternal: identical proposal action already queued at eta'
        );
    ...
    function execute(uint256 proposalId) external {
        require(
            ...
            'VrbsDAO::execute: proposal can only be executed if it is queued'
        );
        Proposal storage proposal = _proposals[proposalId];
    ...
    function cancel(uint256 proposalId) external {
        ...
        require(
            msg.sender == proposal.proposer ||
                vrbs.getPriorVotes(proposal.proposer, block.number - 1) <= proposal.proposalThreshold,
            'VrbsDAO::cancel: proposer above threshold'
        );
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId, 'VrbsDAO::state: invalid proposal id');
        Proposal storage proposal = _proposals[proposalId];
        if (proposal.vetoed) {
    ...
    function castVoteBySig(
        ...
        require(signatory != address(0), 'VrbsDAO::castVoteBySig: invalid signature');
    ...
    function castVoteInternal(
        ...
        require(state(proposalId) == ProposalState.Active, 'VrbsDAO::castVoteInternal: voting is closed');
        require(support <= 2, 'VrbsDAO::castVoteInternal: invalid vote type');
        Proposal storage proposal = _proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, 'VrbsDAO::castVoteInternal: voter already voted');

        /// @notice: Unlike GovernerBravo, votes are considered from the block the proposal was created in order to normalize quorumVotes and proposalThreshold metrics
        uint96 votes = vrbs.getPriorVotes(voter, proposalCreationBlock(proposal));
    ...
    function _setVotingDelay(uint256 newVotingDelay) external {
        ...
        require(
            ...
            'VrbsDAO::_setVotingDelay: invalid voting delay'
        );
    ...
    function _setVotingPeriod(uint256 newVotingPeriod) external {
        ...
        require(
            ...
            'VrbsDAO::_setVotingPeriod: invalid voting period'
        );
    ...
    function _setProposalThresholdBPS(uint256 newProposalThresholdBPS) external {
        ...
        require(
            ...
            'VrbsDAO::_setProposalThreshold: invalid proposal threshold bps'
        );
    ...
    function _setMinQuorumVotesBPS(uint16 newMinQuorumVotesBPS) external {
        ...
        require(
            ...
            'VrbsDAO::_setMinQuorumVotesBPS: invalid min quorum votes bps'
        );
        require(
            newMinQuorumVotesBPS <= params.maxQuorumVotesBPS,
            'VrbsDAO::_setMinQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setMaxQuorumVotesBPS(uint16 newMaxQuorumVotesBPS) external {
        ...
        require(
            ...
            'VrbsDAO::_setMaxQuorumVotesBPS: invalid max quorum votes bps'
        );
        require(
            params.minQuorumVotesBPS <= newMaxQuorumVotesBPS,
            'VrbsDAO::_setMaxQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'VrbsDAO::_setPendingAdmin: admin only');
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), 'VrbsDAO::_acceptAdmin: pending admin only');
    ...
    function _burnVetoPower() public {
        ...
        require(msg.sender == vetoer, 'VrbsDAO::_burnVetoPower: vetoer only');
    ...
    /**
     * @notice Current proposal threshold using Vrb Total Supply
     * Differs from `GovernerBravo` which uses fixed amount
     */
    function proposalThreshold() public view returns (uint256) {
        return bps2Uint(proposalThresholdBPS, vrbs.totalSupply());
    }
     ...
     * @param totalSupply The total supply of Vrbs at the time of proposal creation
    ...
    function getDynamicQuorumParamsAt(uint256 blockNumber_) public view returns (DynamicQuorumParams memory) {
        uint32 blockNumber = safe32(blockNumber_, 'VrbsDAO::getDynamicQuorumParamsAt: block number exceeds 32 bits');
    ...
    /**
     * @notice Current min quorum votes using Vrb total supply
     */
    function minQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).minQuorumVotesBPS, vrbs.totalSupply());
    }

    /**
     * @notice Current max quorum votes using Vrb total supply
     */
    function maxQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).maxQuorumVotesBPS, vrbs.totalSupply());
    }
