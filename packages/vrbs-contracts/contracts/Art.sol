...
/// @title The Vrbs art storage contract
...
contract Art is IArt {
    /// @notice Current Vrbs Descriptor address
    address public override descriptor;

    /// @notice Current inflator address
    IInflator public override inflator;

    /// @notice Vrb Backgrounds (Hex Colors)
    string[] public override backgrounds;

    /// @notice Vrb Color Palettes (Index => Hex Colors, stored as a contract using SSTORE2)
    mapping(uint8 => address) public palettesPointers;

    /// @notice Vrb Bodies Trait
    Trait public bodiesTrait;

    /// @notice Vrb Accessories Trait
    Trait public accessoriesTrait;

    /// @notice Vrb Heads Trait
    Trait public headsTrait;

    /// @notice Vrb Glasses Trait
    ...
    function addPage(
        ...
        trait.storagePages.push(
            VrbArtStoragePage({ pointer: pointer, decompressedLength: decompressedLength, imageCount: imageCount })
        );
    ...
    function imageByIndex(IArt.Trait storage trait, uint256 index) internal view returns (bytes memory) {
        (IArt.VrbArtStoragePage storage page, uint256 indexInPage) = getPage(trait.storagePages, index);
     ...
     * @return IArt.VrbArtStoragePage the page containing the image at index
     * @return uint256 the index of the image in the page
     */
    function getPage(IArt.VrbArtStoragePage[] storage pages, uint256 index)
        internal
        view
        returns (IArt.VrbArtStoragePage storage, uint256)
    {
        uint256 len = pages.length;
        uint256 pageFirstImageIndex = 0;
        for (uint256 i = 0; i < len; i++) {
            IArt.VrbArtStoragePage storage page = pages[i];
    ...
    function decompressAndDecode(IArt.VrbArtStoragePage storage page) internal view returns (bytes[] memory) {
