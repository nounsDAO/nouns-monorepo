...
import {
  NounsBRDAOLogicV2,
  NounsBRDAOLogicV2__factory,
  NounsBRDescriptorV2__factory,
  NounsBRToken,
  Voter__factory,
} from '../../../../typechain';
...
describe('Vote Refund', () => {
  before(async () => {
    ...
    token = await deployNounsBRToken(deployer);
    const descriptor = NounsBRDescriptorV2__factory.connect(await token.descriptor(), deployer);
  ...
  function expectRefundEvent(r: ContractReceipt, u: SignerWithAddress, expectedCost: BigNumber) {
    ...
    const daoInterface = NounsBRDAOLogicV2__factory.createInterface();
