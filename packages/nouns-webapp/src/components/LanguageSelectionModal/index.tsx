        ...
        <div className={classes.LanguageSelectionModal}>
            {
                ...
                .map((locale: Locale) => {
                    return (
                        <div className={classes.languageButton} key={locale.locale} onClick={() => setModalLocale(locale.locale)}>
                            {locale.name}
                            {
                                locale.locale === modalLocale && (
                                    <FontAwesomeIcon icon={faCheck} height={24} width={24} style={{marginTop: '.25rem'}}/>
                                ) 
                            }
                        </div>
                    );
