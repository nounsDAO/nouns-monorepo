...
import { ProposalState } from '../../wrappers/nDao';

import classes from './PunkProfileVoteRow.module.css';
...
import { TokenVoteHistory } from '../ProfileActivityFeed';
...
interface PunkProfileVoteRowProps {
  proposal: Proposal;
  vote?: TokenVoteHistory;
}

const selectIconForNounVoteActivityRow = (proposal: Proposal, vote?: TokenVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: TokenVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
...
const PunkProfileVoteRow: React.FC<PunkProfileVoteRowProps> = props => {
...
export default PunkProfileVoteRow;
